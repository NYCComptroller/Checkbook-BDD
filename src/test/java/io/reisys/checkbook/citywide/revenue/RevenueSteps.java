...
public class RevenueSteps extends CheckbookBaseScenarioSteps {
  ...
  public void validateTotalRevenueWidgetInformation(String yearSelected) throws Exception {
    ...
    String RevenueAmountFromDB = DatabaseUtil
        .getRevenueAmount(ExecutionContext.getJsonData().get(yearSelected).getAsInt(), 'B');
    ...
    assertFieldContainsText("Number of Expense Categories",
        RevenuePage.getWidgetTotalCount("Agencies"), ExpenseCategorieswidgetCountDB.toString());



    // Verify Top 5 RevenueCategories
    Integer RevenueCategorieswidgetCountDB = DatabaseUtil.getRevenueCategoriesCount(year, 'B');
    assertFieldContainsText("Number of Revenue Categories",
        RevenuePage.getWidgetTotalCount("Revenue Categories"),
        RevenueCategorieswidgetCountDB.toString());
    ...
    assertFieldContainsText("Number of Revenue Categories",
        RevenuePage.getWidgetTotalCount("Revenue Categories"),
        RevenueFundingClasswidgetCountDB.toString());
    // Verify Top 5 Programs by committed Expense
  ...
  public void validateRevenueWidgetDetailsInformation(String yearSelected) throws Exception {
    ...
    Integer CitywideProgramsWidgetDetailsCountFromDB =
        DatabaseUtil.getRevenueDetailsCount(year, 'B');
    assertFieldContainsText("Revenue Widget Details Transaction Count",
        RevenuePage.getTotalCountForWidgetDetails(),
        CitywideProgramsWidgetDetailsCountFromDB.toString());
    assertFieldHasText("Revenue Agencies Widget Details Title", RevenuePage.getWidgetDetailTitle(),
        "Agencies Revenue Transactions");
